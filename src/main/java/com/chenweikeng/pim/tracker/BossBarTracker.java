...
public class BossBarTracker {
    ...
    public void update() {
        ...
        PinTraderLocation location = PinTraderRegistry.getInstance().getLocationByWarpPoint(currentWarpPoint);
        ...
        if (targetTrader != null) {
            ...
            if (distance < CLOSE_DISTANCE * CLOSE_DISTANCE && !showingNextTrader && timeSinceLastSwitch >= SWITCH_COOLDOWN_MS) {
    ...
    private void updateDisplayTitle(LocalPlayer player, PinTrader trader) {
        ...
        if (showingNextTrader) {
            displayTitle = Component.literal("Next Pin Trader").setStyle(prefixStyle)
                .append(Component.literal(String.format(" [%.1fm] %s", distance, direction)).setStyle(distanceStyle));
        } else {
            displayTitle = Component.literal("Pin Trader").setStyle(prefixStyle)
                .append(Component.literal(String.format(" [%.1fm] %s", distance, direction)).setStyle(distanceStyle));
        }
    }
    ...
    private Component createNextLocationPrompt(PinTraderLocation nextLocation) {
        ...
        Style textStyle = Style.EMPTY.withColor(ChatFormatting.GOLD).withBold(true).withObfuscated(false);
        
        return Component.literal("\u2588\u2588\u2588\u2588\u2588").setStyle(glitchedStyle)
    ...
    private Component createReturnToFirstLocationPrompt(PinTraderLocation firstLocation) {
        ...
        return Component.literal("\u2588\u2588\u2588\u2588\u2588").setStyle(glitchedStyle)
            .append(Component.literal(" Return to " + firstLocation.getName() + " ").setStyle(textStyle))
            .append(Component.literal("\u2588\u2588\u2588\u2588\u2588").setStyle(glitchedStyle));
    }
