...
public class PimCommand {
    ...
    public static void register() {
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
            dispatcher.register(ClientCommandManager.literal("pim:trade")
                    .executes(context -> {
                        ...
                        if (newState) {
                            ...
                            context.getSource().sendFeedback(Component.literal("§6✨ §e[Pim] §fUse the IFone to warp to the first pin trader."));
                        } else {
                            BossBarTracker.getInstance().disable();
                            context.getSource().sendFeedback(Component.literal("§6✨ §e[Pim] §fPin trading has been stopped."));
                        }
                        return 1;
                    })
            );
