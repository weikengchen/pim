...
public class PinBookHandler {
    ...
    private boolean updateBookEntry(PinBookEntry parsedEntry) {
        ...
        PinRarityHandler.PinSeriesEntry seriesEntry = PinRarityHandler.getInstance().getSeriesEntry(parsedEntry.seriesName);
        if (seriesEntry == null) {
    ...
    private void flushMessages() {
        ...
        if (player != null) {
            if (!pendingMissingSeries.isEmpty()) {
                player.displayClientMessage(Component.literal("§c⚠ §e[Pim] §fSome required pin series information is missing. Please open §e/pinrarity §fto update the pin series information."), false);
            }

            if (!pendingNewSeries.isEmpty()) {
                player.displayClientMessage(Component.literal("§6✨ §e[Pim] §fFound §e" + pendingNewSeries.size() + "§f new pin pack" + (pendingNewSeries.size() == 1 ? "" : "s") + " in your pin book."), false);
            }
        }
